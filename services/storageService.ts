...
const DELETED_DEFAULTS_KEY = 'exam_ai_deleted_defaults_v1';
...
// --- 삭제된 기본 데이터 관리 ---
export const getDeletedDefaultIds = (): Set<string> => {
  ...
  if (saved) {
    try {
      ...
      console.error('Failed to parse deleted defaults from localStorage', e);
    }
  }
  return new Set();
};

export const addDeletedDefaultId = (id: string): void => {
  if (!storageAvailable) return;
  const ids = getDeletedDefaultIds();
  ids.add(id);
  try {
    localStorage.setItem(DELETED_DEFAULTS_KEY, JSON.stringify(Array.from(ids)));
  } catch(e) {
     console.error('Failed to save deleted default ID', e);
  }
};

export const clearDeletedDefaultIds = (): void => {
    if (storageAvailable) {
