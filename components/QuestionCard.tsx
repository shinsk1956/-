...
const QuestionCard: React.FC<QuestionCardProps> = ({ 
  ...
  const cleanTextForTTS = (text: string): string => {
    if (!text) return '';
    // 밑줄, 별표, 해시, 괄호, 화살표(→) 등 음성 출력에 불필요한 특수 기호를 공백으로 치환
  ...
  const playTTS = (textToSpeak: string, type: 'question' | 'explanation') => {
    ...
    // 모바일 브라우저에서 엔진이 멈춰있는 경우를 대비해 깨움
    ...
    // Voices가 비어있는 경우(일부 모바일 크롬 대응) 다시 가져옴
    if (voices.length === 0) {
    ...
    const utterances = segments.flatMap(segment => {
        ...
        return sentences.map(sentence => {
            ...
            const voice = voices.find(v => v.lang === targetLang && (v.name.includes('Google') || v.name.includes('Premium'))) || voices.find(v => v.lang === targetLang) || voices.find(v => v.lang.startsWith(targetLang.split('-')[0])) || null;
            utterance.lang = targetLang;
            if (voice) utterance.voice = voice;
            utterance.rate = 1.1; // 너무 빠르지 않게 조정
  ...
  const toggleQuestionAudio = () => {
    if (isEditing) return;
    const choicesText = availableChoices.map(n => `${n}번, ${localQ.choices[n as 1|2|3|4|5]}`).join('. ');
    const fullText = `문제 ${localQ.questionNumber}번. ${localQ.questionText}. ${choicesText}`;
  ...
  return (
    ...
    >
      ...
      {(isEditing || localQ.questionImage?.trim()) && (
        <div className="mb-8">
          {isEditing ? (
            <div className="flex flex-col">
